...
const Friend = ({friend, handleSelectdFriend,  selectedUser}) => {
  return (
    <li className={selectedUser?.id === friend?.id ? 'selected': ''} key={selectedUser.id}>
        ...
        <Button onHandleClick={()=> handleSelectdFriend(friend.id)}>{selectedUser?.id === friend.id ? 'Selected' : 'Select'}</Button>
    </li>
  )
}
